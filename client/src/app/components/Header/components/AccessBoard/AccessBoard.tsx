...
export const AccessBoard: FC<AccessBoardProps> = ({boardId}) => {
    ...
    return (
        <div className={style.accessBlock}>
            <AccessBoardColumn title={'Доступна'} type={'delete'} data={accessUsers || []} mutation={addUser} boardId={boardId}/>
            <AccessBoardColumn title={'Остальные пользователи'} type={'add'} data={remainingUsers || []} mutation={deleteUser} boardId={boardId}/>
        </div>
    );
};
